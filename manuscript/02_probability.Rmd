...
```{r title="Code for plotting the density", echo=TRUE, fig.keep='none', label = "help_call_density"}
x <- c(-0.5, 0, 1, 1, 1.5)
y <- c(0, 0, 2, 0, 0)
...
```{r , echo=FALSE, ref.label="help_call_density", fig.cap="Help call density", label="triangleDensity"}
```
...
```{r , echo=FALSE, fig.cap="Help call density", label="triangleDensityArea"}
x <- c(-0.5, 0, 1, 1, 1.5)
y <- c(0, 0, 2, 0, 0)
x2 <- c(0, 0.75, 0.75)
y2 <- c(0, 1.5, 0)
plot(x, y, lwd = 3,frame = FALSE, type = "l")
polygon(x = x2, y = y2, col = "lightblue", border = "black", lwd = 3)
```
