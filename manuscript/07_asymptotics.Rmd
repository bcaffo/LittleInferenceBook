...
```{r title="Simulating standard normals", echo=TRUE, fig.cap="Cumulative average from standard normal simulations.", label = "normalLLN"}
n <- 10000
...
```{r title="Simulating many coinflips", echo=TRUE, fig.cap="Cumulative proportion of heads from a sequence of coin flips.", label = "coinLLN"}
...
```{r , echo=FALSE, fig.cap="Result of die CLT simulation.", label = "dieCLT", fig.height=4}
nosim <- 1000
...
```{r , echo=FALSE, fig.cap="Results of the coin CLT simulation.", label = "coinCLT", fig.height=4}
nosim <- 1000
...
```{r , echo=FALSE, fig.cap="Results of the simulation when p=0.9", label = "coinCLT2", fig.height=4}
nosim <- 1000
...
```{r title="Finding a confidence interval.", echo=TRUE}
library(UsingR)
data(father.son)
...
```{r title="Code for investigating Wald interval coverage", echo=TRUE}
n <- 20
...
```{r , echo=FALSE, fig.cap="Plot of Wald interval coverage.", label = "waldCoverage"}
...
```{r , echo=FALSE, fig.cap="Output of simulation with $n=100$.", label = "waldCoverage2"}
...
```{r title="Code for investigating Agresti/Coull interval coverage when n=20.", echo=TRUE}
n <- 20
pvals <- seq(0.1, 0.9, by = 0.05)
nosim <- 1000
coverage <- sapply(pvals, function(p) {
...
```{r , echo=FALSE, fig.cap="Coverage of the Agresti/Coull interval with $n=20$", label = "agrestiCoull"}
ggplot(data.frame(pvals, coverage), aes(x = pvals, y = coverage)) +
  geom_line(size = 2) +
  geom_hline(yintercept = 0.95) +
  ylim(.75, 1.0)
...
```{r title="Code for asymptotic Poisson confidence interval", echo=TRUE}
x <- 5
t <- 94.32
lambda <- x/t
...
```{r title="Code for exact Poisson confidence interval", echo=TRUE}
...
```{r title="Code for evaluating the coverage of the asymptotic Poisson confidence interval", echo=TRUE}
...
```{r , echo=FALSE, fig.cap="Coverage of Poisson intervals for various values of lambda", label = "poissonCoverage"}
ggplot(data.frame(lambdavals, coverage),
...
```{r , echo=FALSE, fig.cap="Coverage of Poisson intervals for various values of lambda and t=1000", label = "poissonCoverage2"}
