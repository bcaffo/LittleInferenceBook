...
```{r title="Loading in and displaying the Galton data", echo=TRUE, message=FALSE, fig.cap="Galton's Data", label = "galton", fig.height=4}
...
```{r title="Using manipulate to explore the mean", echo=TRUE, fig.keep='none', label = "manipulate_histo"}
library(manipulate)
myHist <- function(mu){
...
```{r , echo=FALSE, ref.label="manipulate_histo", fig.cap="Histogram illustration", label = "lsm"}
```
...
```{r , echo=FALSE, fig.cap="Fair coin mass function", label = "fairCoin", fig.height=4.5}
ggplot(data.frame(x = factor(0 : 1),
                  y = c(.5, .5)), aes(x = x, y = y)) +
...
```{r , echo=FALSE, fig.cap="Bar graph of die probabilities", label = "die", fig.height=3}
ggplot(data.frame(x = factor(1 : 6),
                  y = rep(1/6, 6)), aes(x = x, y = y)) +
  geom_bar(stat = "identity", colour = 'black', fill = "lightblue")
```
...
```{r , echo=FALSE, fig.cap="Uniform Density", label = "uniform"}
...
```{r , echo=FALSE, fig.cap="Simulation of normals", label = "normalSimulationMean", fig.height=4}
library(ggplot2)
nosim <- 10000; n <- 10
dat <- data.frame(
...
```{r , echo=FALSE, fig.cap="Simulation of die rolls", label = "dieRollSimulationMean", fig.height=3}
dat <- data.frame(
...
```{r , echo=FALSE, fig.cap="Simulation of coin flips", label = "coinFlipSimulationMean", fig.height=3}
dat <- data.frame(
